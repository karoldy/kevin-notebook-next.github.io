...
- [说说JavaScript中的数据类型?存储上的差别?](/interview/javascript/javascript_1)
- [JavaScript数组的常用方法有哪些?](/interview/javascript/javascript_2)
- [JavaScript字符串的常用方法有哪些?](/interview/javascript/javascript_3)
- [谈谈 JavaScript 中的类型转换机制](/interview/javascript/javascript_4)
- [== 和 === 区别, 分别在什么情况使用](/interview/javascript/javascript_5)
- [深拷贝浅拷贝的区别?如何实现一个深拷贝?](/interview/javascript/javascript_6)
- [说说你对闭包的理解?闭包使用场景?](/interview/javascript/javascript_7)
- [说说你对作用域链的理解?](/interview/javascript/javascript_8)
- [JavaScript原型, 原型链?有什么特点?](/interview/javascript/javascript_9)
- [Javascript如何实现继承?](/interview/javascript/javascript_10)
- [谈谈this对象的理解](/interview/javascript/javascript_11)
- [JavaScript中执行上下文和执行栈是什么?](/interview/javascript/javascript_12)
- [说说JavaScript中的事件模型](/interview/javascript/javascript_13)
- [typeof 与 instanceof 区别](/interview/javascript/javascript_14)
- [解释下什么是事件代理?应用场景?](/interview/javascript/javascript_15)
- [说说new操作符具体干了什么?](/interview/javascript/javascript_16)
- [Ajax原理是什么?如何实现?](/interview/javascript/javascript_17)
- [bind、call、apply 区别?如何实现一个bind?](/interview/javascript/javascript_18)
- [说说你对正则表达式的理解?应用场景?](/interview/javascript/javascript_19)
- [说说你对事件循环的理解](/interview/javascript/javascript_20)
- [DOM常见的操作有哪些?](/interview/javascript/javascript_21)
- [说说你对BOM的理解, 常见的BOM对象你了解哪些?](/interview/javascript/javascript_22)
- [举例说明你对尾递归的理解, 有哪些应用场景?](/interview/javascript/javascript_23)
- [说说 JavaScript 中内存泄漏的几种情况?](/interview/javascript/javascript_24)
- [Javascript本地存储的方式有哪些?区别及应用场景?](/interview/javascript/javascript_25)
- [说说你对函数式编程的理解?优缺点?](/interview/javascript/javascript_26)
- [Javascript中如何实现函数缓存?函数缓存有哪些应用场景?](/interview/javascript/javascript_27)
- [说说 Javascript 数字精度丢失的问题, 如何解决?](/interview/javascript/javascript_28)
- [什么是防抖和节流?有什么区别?如何实现?](/interview/javascript/javascript_29)
- [如何判断一个元素是否在可视区域中?](/interview/javascript/javascript_30)
- [大文件上传如何做断点续传?](/interview/javascript/javascript_31)
- [如何实现上拉加载, 下拉刷新?](/interview/javascript/javascript_32)
- [什么是单点登录?如何实现?](/interview/javascript/javascript_33)
- [web常见的攻击方式有哪些?如何防御?](/interview/javascript/javascript_34)
...
- 在事件中, this指向触发这个事件的对象, 特殊的是, `IE`中的`attachEvent中`的this总是指向全局对象的window
...
1. 创建一个空对象
2. 将空对象的原型, 指向于构造函数的原型 -> 将空对象的`__proto__`指向构造函数的`prototype`
3. 将空对象作为构造函数的上下文(改变this的指向)
4. 执行构造函数代码。如果构造函数返回一个对象, 那么这个对象会被用作 new 表达式的返回值, 否则, 新创建的对象会被返回。
...
- `call()`方法调用一个函数, 并将一个对象作为函数执行时的上下文(this值), 以及一个参数列表传递给函数。
...
JavaScript 中的事件模型是一个核心机制, 允许在特定动作或行为发生时执行预定义的代码。事件可以是用户交互(如点击或键盘输入), 也可以是由浏览器或 JavaScript 代码本身触发的其他动作。

事件模型主要由三部分组成: 

事件源: 这是发生事件的元素或对象。例如, 在 HTML 中, 按钮、链接、输入框等都可以是事件源。
事件类型: 这描述了发生的具体事件类型。例如, `click`、`keydown`、`load`、`mouseover` 等都是常见的事件类型。
事件处理程序: 当事件发生时, 事件处理程序定义了应该执行的代码。事件处理程序通常是一个函数, 这个函数在事件触发时被调用。

而JavaScript事件模型有以下三种模型。

1. 原始事件模型(DOM0级事件模型)
2. DOM2事件模型
3. IE事件模型

[JavaScript事件模型详解](/javascript/docs1/docs_19)
...
Javascrip 中的类型转换可以分为`显示类型转换`和`隐式类型转换`。

(1)显示类型转换是指我们手动调用原生函数进行类型转换, 常见的有 Number()、String()、Boolean()。

还有一个`parseInt`, 相比于Number转换就没有那么严格了, `parseInt`函数逐个解析字符, 遇到不能转换的字符就停下来
...
就是在复制数据或者对象的时候, 将其内存中值复制过来。

**浅拷贝**

就是在复制数据或者对象的时候, 是将其引用复制过来。

**区别：**

深拷贝复制的是被复制数据或者对象的值, 复制的数据或对象会在内存中重新分配内存空间, 赋值的和被赋值的互不影响；浅拷贝赋值的是被复制数据或者对象的引用, 复制的数据或对象通过引用指向被复制数据或者对象引用所指向的值。
...
JavaScript常用的本地缓存的方法有:

- cookie
- localStorage
- sessionStorage

>拓展: indexedDB也是本地存储的一种方式, 但讲解有些复杂, 想详细了解的请查看[indexedDB详解](/javascript/docs3/indexeddb)
