...
完成上述步骤之后, 则开始初始化`Compiler`编译对象, 该对象掌控者webpack声明周期, 不执行具体的任务, 只是进行一些调度工作
...
初始化完成后会调用`Compiler`的run来真正启动webpack编译构建流程, 主要流程如下
...
当完成了上述的`compilation`对象后, 就开始从Entry入口文件开始读取, 主要执行`_addModuleChain()`函数, 如下
...
seal方法主要是要生成`chunks`, 对`chunks`进行一系列的优化操作, 并生成要输出的代码
