...
`terser`是一个JavaScript的解释、绞肉机、压缩机的工具集, 可以帮助我们压缩、丑化我们的代码, 让`bundle`更小
...
使用`HtmlWebpackPlugin`插件来生成HTML的模板时候, 通过配置属性`minify`进行html优化
...
`sideEffects`用于告知`webpack compiler`哪些模块时有副作用, 配置方法是在`package.json`中设置sideEffects属性
