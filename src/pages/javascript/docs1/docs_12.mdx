...
`DOM`像原子包含着亚原子微粒那样，也有很多类型的DOM节点包含着其他类型的节点。接下来我们先看看其中的三种：
...
在以前，我们使用`Jquery`，`zepto`等库来操作DOM，之后在`vue`，`Angular`，`React`等框架出现后，我们通过操作数据来控制DOM（绝大多数时候），越来越少的去直接操作DOM
...
传入任何有效的`css 选择器`，即可选中单个 DOM元素（首个）：
...
TODO 面试题：innerHTML和innerText的区别
